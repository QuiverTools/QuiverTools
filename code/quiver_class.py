...
# This file was *autogenerated* from the file quiver_class.sage
...
class Quiver:
    ...
    def __init__(self, adjacencyMatrix, name=None):
        ...
        assert all([all([(a >= _sage_const_0 ) for a in rows]) for rows in [list(rows) for rows in list(adjacencyMatrix)]])
    ...
    def generalized_Kronecker_quiver(cls, m):
        ...
        return cls(matrix([[_sage_const_0 ,m],[_sage_const_0 ,_sage_const_0 ]]), name = str(m)+"-Kronecker quiver")

    @classmethod
    def three_vertex_quiver(cls, m12,m13,m23):
        """An acyclic quiver with 3 vertices and mij many arrows i --> j for 1 <= i < j <= 3."""
        return cls(matrix([[_sage_const_0 ,m12,m13],[_sage_const_0 ,_sage_const_0 ,m23],[_sage_const_0 ,_sage_const_0 ,_sage_const_0 ]]), name = "An acyclic 3-vertex quiver")
    ...
    def subspace_quiver(cls, m):
        ...
        A = zero_matrix(ZZ,m+_sage_const_1 )
        for i in range(m):
            A[i,m] = _sage_const_1 
    ...
    def thin_dimension_vector(self):
        return vector([_sage_const_1  for i in range(self.number_of_vertices())])
